...
- Reset disables auto-merge only when armed, closes an unmerged pull request, deletes the owned remote branch, removes the owned local worktree and branch, and atomically moves the active Pi session into historical storage only when each action remains required.
