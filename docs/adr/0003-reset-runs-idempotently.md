...
- Reset refuses a merged Run, a live or potentially live Worker, a manually closed issue, or conflicting workflow labels.
- Reset disables auto-merge only when armed, closes an unmerged pull request, deletes its remote branch, removes the local worktree and branch, and retires the resumable Pi session only when each artifact exists.
...
- Reset preserves Run metadata and logs, marks the old Run as reset, and releases its Lease only after required Reset actions succeed.
