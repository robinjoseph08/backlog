...
- `backlog resolve <run-id|positive-issue-number>` provides explicit dry-run, interactive, and `--yes` operation when no Runner is active. In addition to incomplete leased Runs and Historical External Resolution verification, it can select a Historical merged Run to finish verified owned-artifact cleanup without recreating a Lease or changing its Completion metadata. A fully cleaned Historical merged Run is a verification-only no-op rerun. It refuses during active Runner supervision and explains that the supervising Runner handles automatic reconciliation at startup, during watch polling, and after normal Worker settlement. Backlog will not add a separate resolution-request control plane.
...
- Conclusively absent branches, worktrees, and active sessions are already retired. Already archived sessions are also satisfied. Ignored files inside a Backlog-owned worktree are disposable Run-local contents, do not make the worktree changed, and are deleted during retirement. Where Completion cleanup requires a clean worktree, tracked modifications and non-ignored untracked files stop Completion with the Lease retained for an incomplete Run, or with Historical Completion metadata unchanged for a merged Run. Mismatched or unknown artifacts stop resolution under the same ownership policy.
