...
- A merged expected pull request and closed issue always become Completion, even when discovered during External Resolution. Explicit resolution retires the Run's owned branches, worktree, and active Pi session before recording Completion and releasing the Lease instead of deferring cleanup to a future Runner startup.
...
- GitHub and artifact state are revalidated before destructive mutations and finalization. The recorded expected pull request merging changes the outcome to Completion. When the Run stopped before recording a pull request, exactly one merged pull request from its expected branch also establishes Completion; multiple unrecorded merged pull requests remain ambiguous. A pull request from a different branch does not establish Completion. An issue that reopens stops resolution with its Lease retained so the operator can close it again or Reset the Run.
