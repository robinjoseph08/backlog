...
- An incomplete Run may enter `resolving-externally` while retaining its Lease, then become a Historical Run in `resolved-externally` after cleanup. The outcome records when Backlog recognized the resolution and GitHub's issue closure reason.
- A merged expected pull request and closed issue always become Completion, even when discovered during External Resolution.
- A supervising Runner checks incomplete leased Runs at startup, after Worker settlement, and during watch reconciliation. It performs the complete External Resolution automatically when an issue is closed.
