...
func TestRunnerReportsCandidateDiscoveryFailureAndAdmissionRecovery(t *testing.T) {
	...
	github := &fakeGitHub{
		candidateResults: []candidateResult{
			{err: &ghadapter.CandidateDiscoveryError{Operation: "list candidates", Err: listErr}},
			{err: &ghadapter.CandidateDiscoveryError{Operation: "inspect candidate", Issue: 17, Err: inspectErr}},
			{},
		},
		candidateChanged: make(chan struct{}, 3),
	}
	...
	if len(events) != 3 {
		t.Fatalf("operational events = %#v, want two discovery failures and recovery", events)
	}
	...
	if listing.Operation != "list candidates" || listing.Issue != nil || !errors.Is(listing.Err, listErr) || !listing.OccurredAt.Equal(occurredAt) || !listing.RetryAt.Equal(occurredAt.Add(20*time.Millisecond)) || listing.ConsecutiveFailures != 1 {
		t.Fatalf("Candidate listing failure = %#v", listing)
	}
	inspection, ok := events[1].(CandidateDiscoveryFailed)
	if !ok || inspection.Operation != "inspect candidate" || inspection.Issue == nil || *inspection.Issue != 17 || !errors.Is(inspection.Err, inspectErr) || inspection.ConsecutiveFailures != 2 {
		t.Fatalf("Candidate inspection failure = %#v", events[1])
	}
	recovery, ok := events[2].(CandidateDiscoveryRecovered)
	if !ok || recovery.Failures != 2 || !recovery.OccurredAt.Equal(occurredAt) {
		t.Fatalf("recovery = %#v", events[2])
	}
	for _, want := range []string{
		...
		"candidate discovery recovered; admission resumed after 2 failures\n",
	} {
