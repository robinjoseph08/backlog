...
func (p *statusPrinter) run(observed statusRun) {
	...
	case scheduler.StatusResolvingExternally:
		p.printf("    Intervention: External Resolution is incomplete; close the issue if needed and rerun backlog resolve, or Reset the Run with backlog reset; a supervising Runner will retry automatically; Worker not active\n")
		p.printReason(run)
