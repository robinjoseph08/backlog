...
func resolveCommandWithInput(ctx context.Context, args []string, stdin io.Reader, interactive bool, stdout, stderr io.Writer) error {
	...
	flags.Usage = func() {
		...
		fmt.Fprintln(stderr, "")
		fmt.Fprintln(stderr, "Recognize a supported GitHub issue closure as External Resolution of")
		fmt.Fprintln(stderr, "an incomplete leased Run only when owned unmerged pull requests, remote")
		fmt.Fprintln(stderr, "and local branches, worktrees, and active Pi sessions are absent. Remove")
		fmt.Fprintln(stderr, "managed active and Candidate labels, preserve diagnostics, and release the Lease.")
