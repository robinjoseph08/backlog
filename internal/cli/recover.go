...
func recoverCommandWithInput(ctx context.Context, args []string, stdin io.Reader, interactive bool, stdout, stderr io.Writer) error {
	...
	flags.Usage = func() {
		...
		fmt.Fprintln(stderr, "issue, and expected-branch pull request state. Completion and armed auto-merge")
		fmt.Fprintln(stderr, "take precedence. A Run that can Resume must also pass branch, worktree, Pi session,")
