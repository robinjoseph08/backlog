...
func TestPlanStartsOldestEligibleCandidatesWithinCapacity(t *testing.T) {
	...
	plan := Plan(Snapshot{
		...
		Runs: []Run{{Issue: 20, Status: StatusRunning}},
	}, 3)
...
func TestPlanDoesNotStartAnIssueTwice(t *testing.T) {
	t.Parallel()

	plan := Plan(Snapshot{
		Candidates: []Candidate{{Number: 10, CreatedAt: time.Now()}},
		Runs:       []Run{{Issue: 10, Status: StatusFailed}},
	}, 3)
...
func TestPlanCountsOnlyActiveRunsAgainstCapacity(t *testing.T) {
	...
	plan := Plan(Snapshot{
		...
		Runs: []Run{
			{Issue: 1, Status: StatusMerged},
			{Issue: 2, Status: StatusRunning},
		},
	}, 2)
...
func TestPlanWaitingForMergeLeaseDoesNotConsumeWorkerCapacity(t *testing.T) {
	...
	plan := Plan(Snapshot{
		...
		Runs:       []Run{{Issue: 1, Status: StatusWaitingForMerge}},
	}, 1)
