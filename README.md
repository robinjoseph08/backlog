...
Inspect state. Plain status includes each snapshotted issue title when available, falls back to its issue number for older history, and reports `suspending` only while a verified local Runner is supervising bounded suspension. JSON status records `suspendingAt` and `suspendedAt` timestamps for the corresponding lifecycle transitions. Reading version 1 state performs the version 2 migration under the repository lock before printing status:
