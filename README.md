...
The first `SIGINT` enters Drain. Backlog atomically stops admitting new Leases, allows every Run with a committed Lease to settle and reconcile, reports the stage and remaining Worker count, and exits successfully when no owned Workers remain. A Run whose Lease committed just before Drain may still finish worktree preparation and Worker launch. Additional `SIGINT` requests are reported while Drain continues; clean suspension is implemented by the next lifecycle stage.
