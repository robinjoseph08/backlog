...
- Pi 0.80.4 or newer. The default initial prompt requires the global `afk` skill; custom prompts may invoke another skill or no skill.
...
--prompt TEMPLATE    Use an inline initial Worker prompt template
--prompt-file PATH   Read the initial Worker prompt template from a file
```

The concurrency limit counts top-level issues. Each Worker may itself launch implementation and review Subagents.

`--prompt` and `--prompt-file` are mutually exclusive and apply only to fresh Runs admitted by that `backlog run` invocation. With neither flag, Backlog uses the exact compatible template `/skill:afk {{issue_number}}`. An explicitly empty inline prompt, or a missing, unreadable, or empty prompt file, fails startup before Admission. Relative prompt-file paths are resolved from the directory where the operator invoked Backlog, independently of `--repo-dir`. Backlog reads and validates the complete file once during startup, so later file changes cannot alter prompts for later Admissions in the same invocation.

Prompt templates support these built-ins:

```text
{{issue_number}}   Decimal Candidate issue number
{{issue_title}}    Title from the admitted Candidate snapshot
{{issue_url}}      URL from the admitted Candidate snapshot
{{repository}}     Canonical owner/name repository slug
{{default_branch}} Repository default branch
{{run_id}}         Durable Run ID
{{branch}}         Planned isolated branch
{{worktree}}       Planned isolated worktree path
```

Interpolation is strict and performs one pass. Unknown, empty, malformed, or unclosed placeholders fail startup before Admission. Repeated placeholders receive the same value, while placeholder-like text introduced by a value remains literal. Multiline text and Unicode are preserved exactly through Pi's JSON RPC prompt message. The rendered prompt can contain arbitrary instructions and need not invoke or position any particular skill or issue identity.
...
The runner starts Pi in RPC mode in the issue worktree using a deterministic session ID derived from the Run ID and a dedicated session directory under Backlog state. A start gate prevents Pi from opening the session or receiving its initial prompt until the Worker JSONL and standard-error log paths, PID, process-start identity, rendered-prompt digest, and pending prompt-ownership marker are durable.

Backlog renders the selected template once for each fresh Run and submits the exact result as a correlated RPC `prompt` command. The default result for issue `#123` remains `/skill:afk 123`. Pi may transform that command before persisting the user entry, including by expanding a `/skill:` command. After Pi acknowledges the correlated prompt, Backlog reads the fresh session through a separately correlated RPC command, requires exactly one initial user entry, and durably records that entry's ID and canonical content digest before supervising the Worker. Plain text, multiline prompts, the default AFK command, and arbitrary skill commands use the same contract. Standard output uses strict LF-delimited JSONL and is saved separately from standard error. Missing, malformed, ambiguous, mismatched, or duplicate startup evidence and invalid lifecycle ordering fail closed and preserve the worktree.
...
State keeps Historical Runs separate from active Leases. New Runs snapshot the Candidate issue title and URL when their Lease is created, then persist their RPC session identity, dedicated session storage, the SHA-256 digest of the exact rendered initial prompt, and a versioned pending prompt-ownership marker before launch. After correlated startup, the marker gains the actual persisted user entry ID and canonical content digest. Backlog state never stores the prompt template, rendered prompt, expanded skill body, or prompt-file path. Worker log paths become durable as soon as the gated Worker starts. Existing Runs without issue snapshots, startup log paths, prompt digests, or versioned ownership evidence remain valid and are not backfilled through GitHub or mutable skill files. Runner startup and successful lifecycle mutations migrate versions 1 through 6 state atomically to schema version 7. Migration marks those Runs as legacy so only they may use the previous exact prompt rules. Status instead validates and previews the current representation of legacy state versions without persisting migration. Migration preserves Run metadata, Leases, continuation evidence, diagnostics, and artifacts without inventing prompt ownership evidence. Historical `failed` and `needs-human` outcomes remain unacknowledged, while completed Reset outcomes are already treated as handled. Version 1 migration also removes the obsolete paused setting and records legacy print-mode Runs as non-resumable. Incomplete and intervention-required Runs retain their Leases, while verified merged Runs remain as history without active ownership. Legacy print-mode Runs cannot Resume because they have no verified RPC continuation boundary. Backlog refuses state from newer unsupported versions rather than attempting a downgrade.

On restart, the runner reconciles persisted Leases with process liveness and GitHub pull request and issue state before Candidate admission. It automatically performs complete External Resolution for verified closed issues with no Owned Worker, including resuming remaining actions from `resolving-externally`. Watch polling repeats this check during normal reconciliation. When an issue closes during an Owned Worker's lifecycle, periodic reconciliation skips it and the Worker settles normally. After process-group exit and durable log closure, the runner rechecks Completion and immediately performs External Resolution when the issue was handled elsewhere. Safe Suspended Runs fill Worker capacity before new Candidates. Resume rechecks Completion, the open issue and managed workflow labels, the exact branch and worktree, the Pi session identity, owned initial persisted user entry, durable leaf, entry count and file hash, and proof that the old Worker stopped. A replacement Worker keeps the Run, Lease, branch, worktree, and Pi session identities while receiving a new PID and process-start identity. Resume reopens the verified existing Pi session, preserves its actual initial user entry, and sends Backlog's fixed checkpoint-aware reassessment and mutation-safety prompt. It never replays or re-renders the initial prompt, and `--prompt` or `--prompt-file` never customizes Resume.
...
Inspect state. Status is a concise operational projection with Active, Attention Required, Outcomes to Acknowledge, and Recent Completions sections. Active and Attention Required are classified from retained Leases. Every unacknowledged Historical Run in `failed` or `needs-human` remains visible under Outcomes to Acknowledge without an age or count limit. Recent Completions contains the ten newest merged Runs plus any older merged Run whose Completion cleanup is still pending. Completed Reset and External Resolution outcomes, along with explicitly acknowledged outcomes, are omitted from the default projection.

On a terminal, status presents the same projection in compact, width-aware rows with the Run dashboard's semantic colors. Active work is cyan or blue, completions are green, waiting or degraded work is amber, attention is red, and metadata is muted. `NO_COLOR` keeps the compact layout without ANSI styling. Redirected output retains the detailed plain format and never emits ANSI controls.

Every Run displayed by status includes elapsed time, using `n/a` when no start time is available. Active running Runs use Follow's observation model to report verified Worker liveness, Activity age, the current deepest Worker or Subagent operation, separate turn counts, and observed tokens. Missing legacy telemetry is shown as `n/a`. Detailed plain status includes each snapshotted issue title and URL when available, falls back to its issue number for older history, and reports `suspending` only while a verified local Runner is supervising bounded suspension. Sections are newest first with deterministic Run ID tie-breaking. The summary distinguishes total and displayed Runs and reports how many acknowledged outcomes the default projection hides.
