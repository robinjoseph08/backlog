...
Recovery requires absent Runner coordination, a conclusively absent Worker and process group, an open issue with active managed labels, the exact expected branch and worktree, one unambiguous Pi session with a complete tool-result tail, unchanged durable leaf and hash, a supported AFK or ship-it checkpoint stage, and a verified expected-branch pull request state. Existing terminal Runs without a marker may derive one offline only after those absence checks. Dry-run is read-only. Mutation confirms by default, revalidates after confirmation, preserves the original diagnostic and every ownership identity, records Recovery metadata separately from implementation repair attempts, and transitions to Suspended. Normal Resume performs fresh launch checks and sends the exact checkpoint stage in the replacement prompt. Completion and armed auto-merge take precedence. Refusal retains the Lease and artifacts.

A structured top-level Pi `auto_retry_end` event with `success:false` is the only automatic provider-exhaustion signal. After Pi exhausts its own retries, Backlog permits one continuation from a verified durable boundary after a fixed 30-second cooldown. A second provider exhaustion becomes `needs-human`. Provider continuation attempts and explicit Recovery counts are durable and do not consume AFK or ship-it implementation repair budgets.
