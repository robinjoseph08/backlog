...
Candidate discovery fails closed for admission. If a discovery pass fails, the runner creates no Leases from that pass, continues supervising existing Workers, and retries after `--poll`. With `--watch`, it also retries while no Run is active. Without `--watch`, it reports the discovery error once no unfinished leased Run remains.
